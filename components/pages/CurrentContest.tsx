...
export const CurrentContest = () => {
    ...
    const onSubmit: SubmitHandler<Input> = async ({ discordId, discordName }: Input) => {


        try {
            const backToSave = await fetch('/api/contestants/getInToContest', {
                method: 'POST',
                headers: {
            ...
            if (response.Objeto.transaction.resultado === true) {
                router.push('concursante/ingreso-exitoso')
            };

            if (response.Objeto.transaction.resultado === false) {
                router.push('concursante/ingreso-fallido')
            };
